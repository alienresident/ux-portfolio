...
function semplice_post_password($spa) {
	...
	if($frontend_mode == 'dynamic') {
		$submit = '<a class="post-password-submit semplice-event" data-event-type="helper" data-event="postPassword" data-id="' . $post->ID . '">Submit</a>';
	} else {
	...
	if(isset($advanced['password_form_theme']) && $advanced['password_form_theme'] == 'dark') {
		$theme = ' post-password-form-dark';
	}

	// form
	$fullscreen_form = '
		<div class="post-password-form' . $theme . '">
			<div class="inner">
				<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" method="post">
					<div class="password-lock">' . get_svg('frontend', 'icons/password_lock') . '</div>
					<p>This content is protected. <br /><span>To view, please enter the password.</span></p>
					<div class="input-fields">
						<input name="post_password" class="post-password-input" type="password" size="20" maxlength="20" placeholder="Enter password" />' . $submit . '
					</div>
				</form>
			</div>
		</div>
	...
	if(get_post_type($post->ID) == 'page' || get_post_type($post->ID) == 'project' || true === $spa || is_single()) {
		return $fullscreen_form;
	} else {
		return '';
	}
}
